...
import { mealSchedule, mealLogs, accreditationByCode } from "@/shared/api/mock-data";
...
export default function MealTrackingPage() {
  ...
  const filtered = useMemo(
    () => (filter === "ALL" ? mealLogs : mealLogs.filter((m) => m.mealType === filter)),
    [filter]
  );

  const granted = mealLogs.filter((m) => m.result === "ruxsat").length;
  const denied = mealLogs.filter((m) => m.result === "rad").length;
