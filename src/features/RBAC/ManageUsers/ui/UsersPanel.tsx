...
import { systemUsers as initialUsers, roles as initialRoles } from "@/shared/api/mock-data";
...
export function UsersPanel() {
  ...
  const [roleId, setRoleId] = useState(initialRoles[1].id);
            ...
            {users.map((u) => (
              <Tr key={u.id} _hover={{ bg: "surface.700" }}>
                ...
                <Td>
                  <Wrap spacing={1}>
                    {u.roleIds.map((rid) => {
                      const r = initialRoles.find((x) => x.id === rid);
                      return (
                        <WrapItem key={rid}>
